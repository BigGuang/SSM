...
public class ResFileController {
    ...
    public void save(ResFile resFile,HttpServletResponse response) throws Exception{
        if(StringUtils.isNull(resFile.getId())){
            resFile.setId(new MD5().getMD5(UUID.randomUUID().toString()));
        }
        //todo:添加其他业务逻辑

        if(resFileService.getResFileById(resFile.getId())==null){
    ...
    public String edit(@RequestParam(value="id",required=true) String id,HttpServletResponse response) throws Exception{
        ResFile resFile=resFileService.getResFileById(id);
        //todo:添加其他业务逻辑


        JSONObject result=JSONObject.fromObject(resFile);
        //response返回结果
        response.setHeader("Content-type", "text/html;charset=UTF-8");
        response.setCharacterEncoding("UTF-8");
        PrintWriter out=response.getWriter();
    ...
    public void del(@RequestParam(value="id",required=true) String id,HttpServletResponse response) throws Exception{
        int num=resFileService.delete(id);
        //todo:添加其他业务逻辑
