...
public class SysUserController {
    ...
    public void save(SysUser sysUser,HttpServletResponse response) throws Exception{
        if(StringUtils.isNull(sysUser.getId())){
            sysUser.setId(new MD5().getMD5(UUID.randomUUID().toString()));
        }
        //todo:添加其他业务逻辑

        if(sysUserService.getSysUserById(sysUser.getId())==null){
    ...
    public String edit(@RequestParam(value="id",required=true) String id,HttpServletResponse response) throws Exception{
        ...
        JSONObject result=JSONObject.fromObject(sysUser);
        //response返回结果
    ...
    public void del(@RequestParam(value="id",required=true) String id,HttpServletResponse response) throws Exception{
        int num=sysUserService.delete(id);
        //todo:添加其他业务逻辑
